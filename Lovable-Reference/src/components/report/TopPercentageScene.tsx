...
// Generate a pseudo-random but consistent percentage based on mood and period
function calculateTopPercentage(mood: string, period: string): number {
  // Use a simple hash to generate consistent percentages
  const hash = (mood + period).split('').reduce((acc, char) => {
    return ((acc << 5) - acc) + char.charCodeAt(0);
  }, 0);
  
  // Generate percentage between 1-20 for positive moods, 10-40 for others
  const isPositiveMood = ['happy', 'playful', 'excited', 'joyful', 'content', 'calm', 'relaxed']
    .some(m => mood.toLowerCase().includes(m));
  
  if (isPositiveMood) {
    return 1 + Math.abs(hash % 15); // 1-15%
  }
  return 10 + Math.abs(hash % 25); // 10-35%
}
...
export const TopPercentageScene = forwardRef<HTMLDivElement, TopPercentageSceneProps>(
  function TopPercentageScene({
    ...
    // Calculate values once on mount to prevent recalculation
    ...
    useEffect(() => {
      ...
      // Then show the percentage with soft bounce
      const percentTimer = setTimeout(() => setPercentageVisible(true), 600);
      // Then reveal supporting text after 350ms pause
    ...
    return (
      <div ref={ref} className="flex flex-col items-center justify-center h-full text-center px-6">
        ...
        {/* Top percentage badge - fixed width container to prevent layout shift */}
        <div 
        ...
        {/* Supporting text - delayed reveal with fixed dimensions */}
        <div
        ...
        >
          ...
          <p 
            className="text-base text-muted-foreground text-center mb-6 px-4"
        ...
        {/* Share this story button - separate from text block with generous spacing */}
        {onShare && (
          <div
        ...
        {/* Soft bounce keyframes - smoother easing, no overshoot wobble */}
        <style>{`

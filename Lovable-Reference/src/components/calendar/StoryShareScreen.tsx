...
export function StoryShareScreen({
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      try {
        ...
        // Create a muted, darker version for background
  ...
  const generateStoryImage = useCallback(async (): Promise<Blob | null> => {
    try {
      ...
      // Draw background with gradient from dominant color
      ...
      // Card dimensions (centered, Spotify-style)
      const cardWidth = 800;
  ...
  const handleShare = useCallback(async () => {
    ...
    try {
      ...
      const file = new File([blob], `${petName}-mood-story.png`, { type: "image/png" });
  ...
  return createPortal(
    ...
    >
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-4 overflow-hidden">
        {/* Card Preview - Spotify style centered card */}
        <div
          className={cn(

...
export function StickerFrame({ sticker, dateKey }: StickerFrameProps) {
  ...
  const renderSticker = (position: "left" | "right") => {
    ...
    if (IconComponent) {
      // Custom SVG icon with color - sized at 10px
      return (
        <div 
          ...
          style={{ 
            color: iconColor, 
            opacity: 0.9,
            transform: `rotate(${rotation}) scale(0.95)`,
          }}
        >
  ...
  return (
    ...
    // Uses integer pixel positioning for crisp rendering (no subpixel blur)
    // Number safe zone: 2px padding on each side + 3px extra gap = 5px minimum from number edge
    <div 
      className="absolute bottom-[2px] left-0 right-0 pointer-events-none z-[1]"
      style={{ height: "14px" }}
      aria-hidden="true"
    >
      {/* Left sticker - positioned with integer pixels, away from center */}
      <div 
        className="absolute top-1/2"
        style={{ 
          // Fixed pixel position: 0px from left edge (flush)
          // translateY uses -50% but we round to avoid subpixel
          left: "0px",
      ...
      {/* Right sticker - positioned with integer pixels, away from center */}
      <div 
        className="absolute top-1/2"
        style={{ 
          // Fixed pixel position: 0px from right edge (flush)
          right: "0px",
          transform: "translateY(-50%)",
        }}
      >

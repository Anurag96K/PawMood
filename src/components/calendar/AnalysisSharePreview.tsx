...
export function AnalysisSharePreview({
    ...
    // Removed extreme pointer-events: none as it can interfere with some mobile interactions
    // Use the backdrop for event capture instead.
    useEffect(() => {
        if (!isOpen) return;
        // Simple scroll lock
    ...
    const handleClose = useCallback(() => {
        ...
        setTimeout(() => {
            setIsClosing(false);
            onClose();
        }, 10); // Minimal delay as AnimatePresence handles the timing
    }, [isClosing, onClose]);

    // Handle Escape key (Hardware back button)
    useEffect(() => {
        ...
        <AnimatePresence>
            {isOpen && !isClosing && (
                ...
                >
                    ...
                    <motion.div
                        className="relative w-full max-w-lg bg-background rounded-t-3xl pb-safe shadow-xl overflow-hidden"

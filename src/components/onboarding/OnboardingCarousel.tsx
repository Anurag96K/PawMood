...
// Subtle horizontal slide variants
const slideVariants = {
    ...
    >
      ...
      <div className="px-8 pb-8">
        ...
        <div className="flex justify-center gap-2 mb-6">
          {slides.map((_, index) => (
            <button
              ...
              className={`h-2.5 rounded-full transition-all duration-200 ${index === currentSlide
                ? "bg-primary w-6"
        ...
        <div className="flex gap-3">
          ...
          <Button
            className="flex-1 h-[58px] rounded-[22px] font-black text-base bg-primary hover:bg-primary/90 text-white shadow-warm active:scale-[0.97] transition-all"
            onClick={nextSlide}
          >
            {currentSlide === slides.length - 1 ? "Get Started" : "Next"}
            {currentSlide < slides.length - 1 && <ChevronRight className="w-5 h-5 ml-1 stroke-[2.5px]" />}
          </Button>
        </div>
      </div>
    </div>

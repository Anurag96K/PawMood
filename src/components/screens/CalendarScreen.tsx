...
// IMPORTANT: Outer birthday background should ONLY depend on *today* matching a birthday date.
...
const BirthdayThemeBackground = () => {
  return (
    <div className="fixed inset-y-0 left-1/2 -translate-x-1/2 overflow-hidden pointer-events-none z-0 app-container">
...
export function CalendarScreen({ isPremium, isBasic = false, onUpgrade, onNavigateToCamera, onNavigateToProfile, analysisCount = 0, onOpenReport, hasActiveSubscription = false, isReturningUser = false }: CalendarScreenProps) {
  ...
  const { getOptimisticImage, setOptimisticImage } = useOptimisticImage();
  ...
  const [customizeDate, setCustomizeDate] = useState<Date | undefined>(undefined);
  ...
  const handleDayClick = (day: number) => {
    ...
    // BUT only if the user has at least one photo (meaning they've started using the app)
  ...
  const handleReplacePhoto = async (newEntry: MoodEntry) => {
    if (!selectedEntry) return;

    // 1. OPTIMISTIC UPDATE (Instant Feedback)
    // Immediately show the new photo in the viewer and grid
    const updatedEntry = convertToCalendarEntry(newEntry);

    // Preserve the original date/ID but swap content
    const optimisticUpdate = {
      ...selectedEntry,
      photo: newEntry.image_url,
      moodText: newEntry.mood,
      emoji: newEntry.mood_emoji,
      descText: newEntry.mood_description,
      careTipText: newEntry.care_tip,
      // IMPORTANT: Keep the ID of the *original* entry being viewed, 
      // but visually show the *new* entry's content.
      // This mocks a "swap" instantly.
    };

    setSelectedEntry(optimisticUpdate);

    // Cache the new image for the *original* entry ID so the grid updates too
    setOptimisticImage(selectedEntry.id, newEntry.image_url);

    // Force grid refresh
    setRefreshKey(k => k + 1);

    // 2. ACTUAL API CALL (Background)
    // TODO: The actual swap logic should be handled by the modal's onSelect prop
    // which calls the parent's replace function. 
    // This function is just for local UI state.
  };
  ...
  // Handle date changes originating from the Full-Screen Viewer navigation
  const handleViewerDateChange = useCallback((date: Date) => {
    const newYear = date.getFullYear();
    const newMonth = date.getMonth();

    // If the month or year changes, update the background calendar
    if (newYear !== year || newMonth !== month) {
      setInstantChange(true);
      setCurrentDate(new Date(newYear, newMonth, 1));
    }

    // Always sync the selection ring position
    setSelectedDate(date);
  }, [year, month]);

  // Pre-close callback for FullScreenMoodViewer
  // 1. Updates calendar current date to match the viewer's final date
  // 2. Coordinates are handled autonomously by CalendarDayCell via updateExitTarget
  const handlePreClose = useCallback(async (dateKey: string): Promise<DOMRect | null> => {
    ...
    const targetDate = new Date(newYear, newMonth, newDay);

    // Final sync check to ensure target date cell is rendered and selected
    // Note: This triggers the useEffect in CalendarDayCell which updates the exitTargetRef
    const monthChanged = newYear !== year || newMonth !== month;
    if (monthChanged) {
      setCurrentDate(new Date(newYear, newMonth, 1));
    }
    setSelectedDate(targetDate);

    // Wait for the cell to mount and call updateExitTarget
    // Increased delay if month changed to ensure stable layout
  ...
  const getCalendarShadow = () => {
    ...
    const theme = settings.theme;
    if (theme === "pawprint") {
      // Pawprint specific shadow if needed, or default
      return themeConfigs[theme].shadow;
    }
    // Safe lookup for other themes
    return themeConfigs[theme]?.shadow || themeConfigs.beige.shadow;
  };


  // Lock calendar if no active subscription.
  // Strict lock: !isPremium (or !hasActiveSubscription) = Locked.
  // Extra credits do not unlock this.
  ...
  return (
    <div className={cn("h-full overflow-hidden pb-24 relative flex flex-col", getThemeClasses())}>
        ...
        <div className="flex items-center justify-between">
          <div className="flex flex-col">
            <h1 className="text-3xl font-extrabold text-foreground tracking-tight">
              {t("calendarTitle") || "Memories"}
            </h1>
            <p className="text-xs text-muted-foreground mt-0.5">
              {t("calendarSubtitle") || "Your pet's mood journey"}
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="group flex items-center gap-1.5 px-3 py-1.5 rounded-2xl bg-white/40 backdrop-blur-md border border-white/40 active:scale-95 transition-all"
              style={{
                boxShadow: '0 4px 12px rgba(0,0,0,0.05), inset 0 1px 0 rgba(255,255,255,0.8)',
              }}
            >
              <Palette className="w-3.5 h-3.5 text-primary" />
              <span className="text-[11px] font-bold text-foreground/80">Decorate</span>
            </button>
            <button
              data-calendar-control="true"
            ...
            >
              ...
              <span className="text-[11px] font-bold text-foreground/80">Report</span>
            </button>
          </div>
        </div>
      </header>
      ...
      {/* Full Screen Mood Viewer */}
      {selectedEntry && selectedDay && !showCustomizeModal && (
        <FullScreenMoodViewer
          ...
          onClose={(finalDateKey) => {
            ...
            if (finalDateKey) {
              ...
              // If no final key returned (e.g. simple close), just clear selection
              setSelectedDate(null);
            }
          }}
      ...
      <CalendarCustomizeModal
        ...
        onClose={() => {
          ...
          // If NOT selected day, clear the temp customization date
          if (!selectedDay) setCustomizeDate(undefined);
        }}

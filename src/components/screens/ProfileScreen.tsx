...
import { Crown, ChevronRight, Settings, HelpCircle, LogOut, Zap, Check, Globe, Lock, User, RefreshCw } from "lucide-react";
...
export function ProfileScreen({
  ...
  scrollToExtraCredits = false
}: ProfileScreenProps) {
  const { t, language, setLanguage } = useLanguage();
  const { profile, avatarSrc } = useProfile();
  const { addExtraCredits } = useExtraCredits();
  const { offerings, getPrice, restore, isLoading: subLoading } = useSubscription();
  const planSectionRef = useRef<HTMLDivElement>(null);
  const extraCreditsSectionRef = useRef<HTMLDivElement>(null);
  ...
  const handleBillingCycleChange = (newCycle: "monthly" | "yearly") => {
    ...
    // Monthly is left, Yearly is right
    // Moving to yearly = moving right = positive direction
    // Moving to monthly = moving left = negative direction
  ...
  // Plan pricing configuration from offerings
  const pricingConfig = {
    monthly: {
    ...
    yearly: {
      ...
      credits: 70,
      monthlyEquivalent: offerings.yearly 
        ? offerings.yearly.product.currencyCode + " " + (offerings.yearly.product.price / 12).toFixed(2)
        : "$7.49",
    },
  };
  ...
  return (
    <div className="min-h-full bg-background pb-24">
      ...
      <div className="px-5 mb-6" ref={extraCreditsSectionRef}>
        <div className="relative px-4 py-5 rounded-xl border-2 border-primary/60 overflow-hidden"
          style={{
      ...
      <div className="px-5">
        ...
        <div className="bg-card rounded-2xl border border-border/50 overflow-hidden shadow-sm">
          {[
            ...
            {
              icon: RefreshCw, label: t("restorePurchases"), action: async () => {
                await restore();
              }
            },
